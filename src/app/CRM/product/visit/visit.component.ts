...
export class VisitComponent implements OnInit {
    ...
    contents: FormArray;
    custDataFields = ['Customer Name', 'Data','Time','Staff Name','Reference','Address', 'Whatsapp', 'Phone 1', 'Phone 2', 'EMAIL', 'Facebook ID'];
    // list mode
    members: any[];
    ...
    getData() {
        this._eCRMFSService.getCustomers('visit')
            .subscribe(members => {
    ...
    getFilterData(filter) {
        this._eCRMFSService.getFilterCustomers('visit', filter)
            .subscribe(members => {
                this.members = members;
                this.dataSource = new MatTableDataSource(members);
                this.dataSource.paginator = this.paginator;
    ...
    setData(data: any) {
        return this._eCRMFSService.setCustomer('visit',data);
    }
    ...
    getCustomerDoc(docId) {
        this.myCustID = docId;
        this.myDocData = this._eCRMFSService.getCustomerDoc('visit', docId);
        this.customerForm.reset();
        this.toggle('editMode');
    }
    deleteCustomerDoc(docId) {
        if (confirm("Are you sure want to delete this record ?")) {
            this.myDocData = this._eCRMFSService.deleteCustomerDoc('visit', docId);
        }
    }
    updateCustomerDoc(docId, formData) {
        this.myDocData = this._eCRMFSService.updateCustomerDoc('visit', formData, docId);
        this.customerForm.reset();
        this.toggle('resMode');
    }
