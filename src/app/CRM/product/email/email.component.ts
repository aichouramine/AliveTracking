...
export class CEmailComponent implements OnInit {
    currentDate;
    currentDate7;
    data: any;
    myDocData: any;
    ...
    contents: FormArray;
    custDataFields = ['Customer Name', 'Request','Item Required','Date Needed','Reference','Address', 'Whatsapp', 'Phone 1', 'Phone 2', 'EMAIL', 'Facebook ID'];
    // list mode
    members: any[];
    ...
    getData() {
        this._eCRMFSService.getCustomers('email')
            .subscribe(members => {
    ...
    getFilterData(filter) {
        this._eCRMFSService.getFilterCustomers('email', filter)
            .subscribe(members => {
                this.members = members;
                this.dataSource = new MatTableDataSource(members);
                this.dataSource.paginator = this.paginator;
    ...
    setData(data: any) {
        return this._eCRMFSService.setCustomer('email',data);
    }
    ...
    getCustomerDoc(docId) {
        this.myCustID = docId;
        this.myDocData = this._eCRMFSService.getCustomerDoc('email', docId);
        this.customerForm.reset();
        this.toggle('editMode');
    }
    deleteCustomerDoc(docId) {
        if (confirm("Are you sure want to delete this record ?")) {
            this.myDocData = this._eCRMFSService.deleteCustomerDoc('email', docId);
        }
    }
    updateCustomerDoc(docId, formData) {
        this.myDocData = this._eCRMFSService.updateCustomerDoc('email', formData, docId);
        this.customerForm.reset();
        this.toggle('resMode');
    }
