...
export class GuestComponent implements OnInit {
    ...
    contents: FormArray;
    custDataFields = ['Father Name', 'Mother Name', 'Wife Name', 'Gender', 'Married', 'Reference', 'DOB', 'PAN', 'AADHAR', 'Passport', 'DL', 'Whatsapp', 'Phone 1', 'Phone 2', 'EMAIL', 'Facebook ID'];
    // list mode
    members: any[];
    dataSource: MatTableDataSource<any>;
    @Output() guestUpdated = new EventEmitter();
    ...
    guestUpdatedDoc(docId) {
        this.guestUpdated.emit(docId);
    }

    getData() {
        this._eCRMFSService.getCustomers('guest')
            .subscribe(members => {
    ...
    getFilterData(filter) {
        this._eCRMFSService.getFilterCustomers('guest', filter)
            .subscribe(members => {
                this.members = members;
                this.dataSource = new MatTableDataSource(members);
                this.dataSource.paginator = this.paginator;
    ...
    setData(data: any) {
        return this._eCRMFSService.setCustomer('guest',data);
    }
    ...
    getCustomerDoc(docId) {
        this.myCustID = docId;
        this.myDocData = this._eCRMFSService.getCustomerDoc('guest', docId);
        this.customerForm.reset();
        this.toggle('editMode');
    }
    deleteCustomerDoc(docId) {
        if (confirm("Are you sure want to delete this record ?")) {
            this.myDocData = this._eCRMFSService.deleteCustomerDoc('guest', docId);
        }
    }
    updateCustomerDoc(docId, formData) {
        this.myDocData = this._eCRMFSService.updateCustomerDoc('guest', formData, docId);
        this.customerForm.reset();
        this.toggle('resMode');
    }
