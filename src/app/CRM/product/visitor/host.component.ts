...
export class HostComponent implements OnInit {
    ...
    displayedColumns = ['customerid', 'name', 'phone', 'key'];
    ...
    getData() {
        this._eCRMFSService.getCustomers('host')
            .subscribe(members => {
    ...
    getFilterData(filter) {
        this._eCRMFSService.getFilterCustomers('host', filter)
            .subscribe(members => {
                this.members = members;
                this.dataSource = new MatTableDataSource(members);
                this.dataSource.paginator = this.paginator;
    ...
    setData(data: any) {
        return this._eCRMFSService.setCustomer('host',data);
    }
    ...
    deleteCustomerDoc(docId) {
        if (confirm("Are you sure want to delete this record ?")) {
            this.myDocData = this._eCRMFSService.deleteCustomerDoc('host', docId);
        }
    }
    updateCustomerDoc(docId, formData) {
        this.myDocData = this._eCRMFSService.updateCustomerDoc('host', formData, docId);
        this.customerForm.reset();
        this.toggle('resMode');
    }
